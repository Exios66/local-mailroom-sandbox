...
def test_sorter_local_variant_mentions_json():
    ...
    assert "extract_local_v0" in list_variants()
    extract = load_variant("extract_local_v0")
    assert "json" in extract.lower()
    assert "null" in extract.lower()
...
def test_merger_specialist_simplified_prompt_is_maud_not_cuad():
    path = Path(__file__).resolve().parents[1] / "config" / "prompts" / "merger_agreement_specialist_simplified.txt"
    assert path.is_file()
    text = path.read_text(encoding="utf-8")
    assert "merger-agreement specialist" in text.lower() or "merger agreement" in text.lower()
    assert "maud" in text.lower()
    assert "effective_time" in text
    assert "cuad_family" in text  # prohibition
    assert "You are a legal extraction specialist focused on commercial contracts" not in text
