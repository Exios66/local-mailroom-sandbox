...
`v0.11.0`; prompt loops optionally in `llm-entity-extraction`.
...
- Scoring is pinned to `llm-dojo-scoring @ v0.11.0` (formulas / T0 names unchanged from v0.10.0; new `llm_dojo_scoring.prompts` catalog). The `llm-mailroom` **v0.5.0 tag** still depends on dojo v0.7.0, so mailroom is not a core pip dependency (pip cannot satisfy both). `sandbox fetch-deps` clones the v0.5.0 source tree; `pip install -e ".[pipeline]"` installs current mailroom *main* (matching family pin once mailroom lands `@v0.11.0`). Local overlay templates stay in `config/prompts/`.
